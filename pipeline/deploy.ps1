##############################################################################
# deploy.ps1 - Full deployment to VPS after Loxo ingest is complete
#
# Run from repo root:
#   .\pipeline\deploy.ps1
#
# Prerequisites:
#   - Loxo Qdrant upsert finished successfully
#   - SSH key present at $KEY_PATH below (stored in ~/.ssh, NOT in repo)
...
# -- 4. Upload DuckDB ---------------------------------------------------------
Step "Uploading DuckDB (this may take a few minutes)"
$localDb  = Join-Path $ROOT "Database\talent_search.duckdb"
$remoteDb = "/var/www/talentin/database/talent_search.duckdb"
if (-not (Test-Path $localDb)) { Fail "DuckDB not found at $localDb" }
$sizeMB = [math]::Round((Get-Item $localDb).Length / 1MB, 1)
Write-Host "  Uploading $sizeMB MB ..."
scp -i $KEY_PATH -o StrictHostKeyChecking=no $localDb "${SERVER}:${remoteDb}"
if ($LASTEXITCODE -ne 0) { Fail "DuckDB upload failed" }
Ok "DuckDB uploaded"

# -- 5. Push backend source files ---------------------------------------------
...
# -- 6. Ensure DEFAULT_CLIENT_ID=00 is in VPS .env ---------------------------
